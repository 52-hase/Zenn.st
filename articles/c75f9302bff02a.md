...
# Rails で LINE ログインを実装する手順

この記事では、以下の流れに沿って実装を行います。
...
class LineLoginApiController < ApplicationController
  ...
  def login
    ...
    client_id = ENV['LINE_LOGIN_CLIENT_ID'] # 環境変数推奨
...
- LINEログインチャネルの作成
- Rails コントローラの実装
- 認可コード → ID トークン → LINEユーザーID の取得
- ngrok による HTTPS 化
- LINE Developers でコールバックURL設定
-動作確認方法

Rails でも LINE ログインは OAuth2 の基本に沿って実装でき、手順さえおさえればシンプルです。