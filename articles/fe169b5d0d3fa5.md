...
CSRFトークンは、Webアプリケーションの脆弱性「クロスサイトリクエストフォージェリ（CSRF）」を防ぐために使用される文字列。

CSRFは、悪意のあるサイトからPOSTリクエストを送信されることで発生する脆弱性で、特定の信頼できるサイトからのみリクエストを受け付けるべき。

他のサイトから自分のサイトにPOSTリクエストを送るのは簡単で、フォームのaction属性やJavaScript、curlなどを使って実現可能。

CSRFトークンを使って、自分のサイトからのPOSTリクエストのみ受け付けるようにする。

ランダムなトークンを生成し、セッションに保存。

フォームに隠しフィールドとして埋め込む。

送信されたリクエストがセッションのトークンと一致する場合のみ正規のリクエストとして受け入れる。

CSRFは広く知られている脆弱性で、一般的なWAFやフレームワーク（Laravelなど）では対策が組み込まれている。

Laravelでは、すべてのPOSTリクエストにCSRFトークンが要求され、@csrfを忘れるとエラーが発生する。
