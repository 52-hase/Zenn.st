...
**コンフリクトが発生したファイルを確認**（`git status`でチェック）
　　↓
**コンフリクト箇所を手動で編集**
　　↓
**`<<<<<<<`、`=======`、`>>>>>>>`の記号を削除**
...
　　↓
**修正後に `git add` → `git commit`**（マージツールの活用も可能）
　　↓
**解決後にテストを実行**（動作確認）
...
**`作業`: ファイルを編集・作成**
　　↓
**`add`: 変更をステージングエリアに追加（`git add .` ⇦ 全ての変更をステージング）**
　　↓
**`commit`: 変更を記録（`git commit -m 'メッセージ'`）**
　　↓
**`push`: リモートリポジトリに送信（`git push origin [branch名]`）**
...
**`develop`から新機能用の`featureブランチ`を作成**
　　↓
**機能開発完了後、`feature`を`develop`にマージ**
　　↓
**リリース時に`releaseブランチ`を作成**
　　↓
**テスト完了後、`release`を`master`と`develop`にマージ**
　　↓
**緊急時は`master`から`hotfixブランチ`を作成**
...
# GitHubのワークフロー（GitHub Actions Workflow）

[概要]  
GitHub Actions Workflow とは、**GitHub上での自動化処理の流れ** を定義する仕組みです。  
テストの実行、ビルド、デプロイ、Lintチェックなどを、イベント（push、pull request、スケジュール実行など）に応じて自動で実行できます。

[構成要素]  
- **Workflow**: 自動化処理の定義ファイル（`.github/workflows/`配下にYAML形式で保存）  
- **Job**: Workflow内で実行される処理のまとまり  
- **Step**: Job内で順に実行される個別の処理（コマンドやAction呼び出し）  
- **Action**: 再利用可能な処理モジュール（公式・コミュニティ製が利用可能）

<br>

[Workflowファイル例] 

**`mainブランチ`にpushされたらテストを自動実行する例**
```yaml:yaml
name: Run Tests
...
**`master`から新しい`feature/作業用ブランチ`を作成**
　　↓
**ローカルで開発作業を実施**
　　↓
**作業完了後、プルリクエストを作成**
