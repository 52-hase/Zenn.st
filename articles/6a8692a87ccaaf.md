...
リロード時は、サーバーがcurrent_userを把握しているため、メッセージを正しく左右に振り分けられる
一方、ActionCable経由のリアルタイム更新では、サーバーは「誰が投稿したか」は分かっても「今誰が画面を見ているか」は分かりません。そのため、送信者・受信者を問わず全員に同じHTMLがbroadcastされ、「自分/他人」の判定が投稿者視点のまま固定されてしまいます。結果として、受信した全員に同じ見た目(例:全員左寄せ)で表示されてしまう