...
class LiveRoomChannel < ApplicationCable::Channel
  ...
  def speak(data)
    ...
    message = live_room.messages.new(
...
２の方が正しく動くかつ安全
live_room_channel.rb
user: current_user
live_room_channel.js
user_id を送信しない
show.html.erb
      ...
      <div class="d-flex align-items-start message-row">
        <% if message.user.avatar.attached? %>
          <img
            ...
            src="<%= url_for(message.user.avatar) %>"
        ...
        <div class="message-content">
          <div class="text-white">
            <%= message.user.name %>
            <span style="color: rgba(108, 117, 125, 10); margin-left: 0.5rem;">
              <%= message.created_at.strftime("%Y-%m-%d %H:%M") %>
            </span>
          </div>

          <div class="message-bubble mt-2">
            <div class="card-text" style="color: black; white-space: pre-wrap;">
              <%= message.content %>
            </div>
...
app/views/live_rooms/show.html.erb は、
CSSクラス方式に変更する場合は最後の<script>タグを削除するだけ
```
<div class="container mt-5 d-flex flex-column align-items-center live-room-container">
  <h2 class="text-center mb-4 live-room-title">
    <%= @live_room.name %>
  ...
  <div class="fixed-bottom bg-light py-3">
    <div class="container">
      <div class="form-group d-flex">
        ...
        <label for="chat-image" id="chat-image-label" class="btn btn-outline-secondary">
          画像
        </label>

        <input type="file" id="chat-image" class="form-control-file d-none" />

        <button id="button" class="btn btn-outline-primary">
          送信
        </button>
        ...
        <div
          id="share-button"
          data-room-name="<%= @live_room.name %>"
          data-room-url="<%= live_room_path(@live_room) %>"
        >
          <button id="share-to-x">
            <%= image_tag 'logo-black.png', alt: 'X Logo', style: 'width: 15px; height: 15px;' %>
            でライブをシェアする
