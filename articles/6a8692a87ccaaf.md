...
受講していたプログラミングスクール「RUNTEQ」では、RubyとRuby on Railsの基礎文法からフレームワークを用いた実践的なWebアプリ開発と、Webアプリをデプロイするまでのプロセスを学びました。
README作成による要件・仕様の整理、画面遷移図やER図による設計、issueベースでのタスク管理を通じた開発、そしてMVPとしてのリリースという流れです。このようなプロセスを経て、アプリを作成しました。
