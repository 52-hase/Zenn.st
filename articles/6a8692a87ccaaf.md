...
// ===============================
// HTML(data-user-id)からログイン中のユーザーIDを取得
// show.html.erbで設定した値を取得する
// ===============================
function getUserId() {
  const userIdElement = document.getElementById("user-id");
  return userIdElement ? userIdElement.dataset.userId : null;
...
function applyMessagePosition(li) {
  // メッセージ行(.message-row)を取得
  const row = li.querySelector(".message-row");
  if (!row) return;

  // メッセージ送信者とログインユーザーを比較
...
// ライブルームIDが取得できた場合のみActionCableへ接続
if (liveRoomId) {
