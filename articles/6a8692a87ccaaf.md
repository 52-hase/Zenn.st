...
受講していたプログラミングスクール「RUNTEQ」では、Webアプリをデプロイするまでのプロセスとして、README作成による要件・仕様の整理、画面遷移図やER図による設計、issueベースでのタスク管理を通じた開発、そしてMVP(実用最小限の製品)としてのリリースという流れを学びました。このようなプロセスを経て、アプリを作成いたします。