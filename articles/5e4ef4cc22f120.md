...
「送信」ボタンをクリック
使用技術
Rails Viewチャット画面のHTMLを描画
Bootstrap入力フォームやボタンのUIを整える
JavaScript送信イベントを検知、ページ遷移せず処理を実行

② JavaScriptがWebSocket通信を開始
JavaScriptが ActionCableのChannelに接続
入力されたメッセージを WebSocket経由で送信
ポイント
...
使用技術
JavaScriptDOM操作でメッセージを追加
Bootstrap吹き出し・レイアウトを統一
結果
