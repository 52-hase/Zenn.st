...
#### ③ ActionCable Client（WebSocket送信）
・`WebSocket`でRailsへ再生リクエスト送信
・`HTTP`ではなく`常時接続通信`を利用
・ページリロードなし

使用技術：
`Action Cable`
`ActionCable`を用いて`WebSocket`の常時接続を確立し、ルーム単位でのリアルタイム同期（チャット・楽曲再生）を実現している。
...
Rails側から：
・`rspotify`経由でSpotifyへアクセス
・楽曲情報を取得
・必要に応じて`再生制御API`を実行

使用API：
`Spotify Web API`
Spotify Web APIを用いて楽曲情報の取得および再生制御を行い、その再生状態を`WebSocket`経由でルーム全体に同期させている。
...
`① spotify_track_id`
役割
・`Spotify`上の楽曲を一意に識別するID
・`Spotify Web API`との連携時に使用
・再生制御（再取得・停止・同期）に必要

ポイント
→ `外部API`と`DB`を紐づける「キー情報」
...
`⑤ ルームID`
役割
・どのチャットルームで再生されているかを識別
・ルーム単位で再生状態を管理
・ActionCableのブロードキャスト対象決定に使用
